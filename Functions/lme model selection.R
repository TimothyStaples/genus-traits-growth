lme.model.selection<-function(response,
                          main.fixed, 
...
# SUBSET AND TEST INTERACTIONS ####
  ...
  int.list<-lapply(interactions,
                   function(x){update(m1, as.formula(paste0(".~. -",x)))})
...
# SUBSET AND TEST NON-INTERACTING MAIN EFFECTS ####
  ...
  if(length(main.effects)!=0){
      ...
      # Get list of models
    main.list<-lapply(main.effects, 
                      function(x){update(m2, 
                                         as.formula(paste0(".~. -",x)))})
