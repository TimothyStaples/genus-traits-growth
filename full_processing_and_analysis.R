...
#         EXTRACT ENVIRONMENTAL CONDITIONS AT PLOTS ####
  ...
  shape.file.dir<-"/home/timothy/University files - offline/PhD - offline/Shape files"
...
mixed.site$density<-mixed.site$plant.count/mixed.site$plot.area
hist(mixed.site$density, breaks=100)
...
mixed.plant$species[mixed.plant$species %in% 
                          species$species[species$unknown==1]]="Unknown"
summary(mixed.plant$species)

ssmat<-table(mixed.plant$pl.p, mixed.plant$species)

# See how number of stems per plot is distributed
hist(mixed.site$plant.count[mixed.site$plant.count<100], breaks=100)
...
unique(rare.rich$pl.p[is.na(match(rare.rich$pl.p, mixed.site$pl.p))])
unique(mixed.site$pl.p[is.na(match(mixed.site$pl.p, rare.rich$pl.p))])

mixed.site<-merge(mixed.site, rare.rich)
colnames(mixed.site)

#                     EXPLORE RICHNESS ####

# look at correlation between richness, evenness and plant count
with(mixed.site, cor(cbind(alpha, rare.rich, plant.count)))

plot(alpha ~ log(mixed.site$plant.count))
plot(rare.rich$rare.rich ~ log(mixed.site$plant.count))
...
mixed.plant<-merge(mixed.plant, trait.means[,colnames(trait.means) %in%
...
trait.n<-as.data.frame(matrix(0, ncol=4, nrow=length(mixed.plant[,1]), 
                              dimnames=list(NULL, 
                                            paste0(colnames(mixed.plant)[grepl("SLA|WD|SM|MH",
                                                                  colnames(mixed.plant))], ".n"))))
# I also want the radius of the circle that was used to calculate genus means
trait.dist<-as.data.frame(matrix(0, ncol=4, nrow=length(mixed.plant[,1]), 
                                 dimnames=list(NULL, 
                                               paste0(colnames(mixed.plant)[grepl("SLA|WD|SM|MH",
                                                                                  colnames(mixed.plant))], ".dist"))))
mixed.plant<-cbind(mixed.plant, trait.n, trait.dist)

#                     CALCULATE GENUS MEANS ####
mixed.plant.save<-mixed.plant
mixed.site.save<-mixed.site
# now here we want to set genus means, but universal means are a bad idea, 
# especially in genera with a lot of variation (e.g., Eucalyptus). So what we'll
# do is a hierarchical process. First, I'm assuming if there's another species
# in the sample plot as a genus ID, the data collector recognised the genus ID 
# was NOT the same species, so I don't want to apply the species trait values 
# to the genus ID. Instead what I'll do is apply a mean from a discreet region.

# for starters we want all genus IDs, and species without trait values
genus<-unique(mixed.plant$species[mixed.plant$genus.only==1])

species.missing.trait<-
  unique(mixed.plant$species[
    rowSums(is.na(mixed.plant[, c("SLA","WD","SM","MH")]))>0 &
      mixed.plant$genus.only==0 &
      mixed.plant$unknown==0])

# next thing we need is a distance matrix for our plantings
  grid.plp<-expand.grid(mixed.site$pl.p, mixed.site$pl.p, stringsAsFactors=FALSE)
  grid.longlat<-data.frame(long1=mixed.site$long[match(grid.plp[,1], mixed.site$pl.p)],
                           lat1=mixed.site$lat[match(grid.plp[,1], mixed.site$pl.p)],
                           long2=mixed.site$long[match(grid.plp[,2], mixed.site$pl.p)],
                           lat2=mixed.site$lat[match(grid.plp[,2], mixed.site$pl.p)])
  ...
  pl.p.dist<-matrix(d, ncol=length(mixed.site$pl.p), nrow=length(mixed.site$pl.p),
                    dimnames=list(mixed.site$pl.p, mixed.site$pl.p),
                    byrow=TRUE)
...
species.by.pl.p<-split(mixed.plant, f=mixed.plant$pl.p)
...
clusterExport(varlist=c("mixed.plant", "pl.p.dist", "species", "species.by.pl.p"))

#summary(mixed.plant$species)
#mixed.plant$pl.p[mixed.plant$species=="Eucalyptus sp."]
#row.num<-match("ACOP8.c", names(species.by.pl.p))
genus.means.list<-parLapply(cl=cl, 1:length(species.by.pl.p), function(row.num){
  ...
  pl.p.distances<-pl.p.dist[rownames(pl.p.dist) == names(species.by.pl.p)[row.num],]
  ...
  pl.p.mean.traits<-lapply(missing.genus, function(genus){
    ...
    cand.traits<-t(sapply(c(50,100,200,500,1000,10000), function(radius){
      ...
      pl.p.radii<-mixed.plant[
        mixed.plant$pl.p %in%
          names(pl.p.distances)[pl.p.distances<radius] &
          mixed.plant$species %in% candidate.species,]
  ...
  colnames(pl.p.mean.traits.df)<-c(colnames(species.traits)[-1],
                                   paste0(colnames(species.traits)[-1], ".n"),
                                   paste0(colnames(species.traits)[-1], ".dist"))
...
mixed.plant<-rbind(mixed.plant[mixed.plant$pl.p %in% 
                                         null.pl.p.names,],
                       do.call("rbind", pl.p.missing.traits[null.pl.p==0]))

write.csv(mixed.plant, "./Data/mixed plant tree.csv")
write.csv(mixed.site, "./Data/mixed site tree.csv")
...
mixed.plant<-read.csv("./Data/mixed plant tree.csv")
mixed.site<-read.csv("./Data/mixed site tree.csv")
...
euc.cent <- plant.cent
euc.cent$genus <- relevel(euc.cent$genus, ref="Eucalyptus")
...
# predict from model to show differences in slope at particular points on interaction plane

model.vars<-rownames(summary(genera.m.arid)$coefficients)
model.vars<-model.vars[!grepl(":|Intercept", model.vars)]
model.vars[model.vars=="genusEucalyptus"] <- "genus"
SLA.df<-as.data.frame(matrix(0, nrow=200, ncol=length(model.vars)))
colnames(SLA.df)<-model.vars

SLA.df.list<-list(SLA.df, SLA.df)

SLA.df.list<-lapply(1:length(SLA.df.list), function(x){
  temp<-SLA.df.list[[x]]
  temp[,"aridity.index"]<-arid.quantile[x]
  
  if(x==1){
    temp[,"SLA"]<-with(plant.cent[plant.cent$genus=="Acacia" &
                                    plant.cent$aridity.index > quantile(plant.cent$aridity.index, 0) &
                                    plant.cent$aridity.index < quantile(plant.cent$aridity.index, 0.2),], 
                       seq(min(SLA), max(SLA), length.out=200))
  } else {
    temp[,"SLA"]<-with(plant.cent[plant.cent$genus=="Acacia" &
                                    plant.cent$aridity.index > quantile(plant.cent$aridity.index, 0.8) &
                                    plant.cent$aridity.index < quantile(plant.cent$aridity.index, 1),], 
                       seq(min(SLA), max(SLA), length.out=200))  
  }
  
  temp[,"genus"]<-factor("Acacia")
  levels(temp[,"genus"])<-c("Acacia", "Eucalyptus")
  return(temp)
})

SLA.preds<-lapply(SLA.df.list, function(x){
  data.frame(predict(genera.m.arid, newdata=x, se.fit=TRUE, re.form=NA))
})

dry.SLA.seq<-with(plant.uncent[plant.cent$genus=="Acacia" &
                                 plant.cent$aridity.index > quantile(plant.cent$aridity.index, 0) &
                                 plant.cent$aridity.index < quantile(plant.cent$aridity.index, 0.2),], 
                  seq(min(SLA), max(SLA), length.out=200))

moist.SLA.seq<-with(plant.uncent[plant.cent$genus=="Acacia" &
                                   plant.cent$aridity.index > quantile(plant.cent$aridity.index, 0.8) &
                                   plant.cent$aridity.index < quantile(plant.cent$aridity.index, 1),], 
                    seq(min(SLA), max(SLA), length.out=200))

screen(5)
par(mar=c(0,0,0,0), ps=8, tck=-0.04, mgp=c(3,0.4,0), las=1)
plot(x=NULL, y=NULL, xlim=c(2,25),
     ylim=c(-0.1, 0.5), axes=FALSE, xlab="", ylab="")

axis(side=1, at=c(5, 10, 15, 20), mgp=c(3,0,0))
axis(side=2, at=c(0, 0.2, 0.4))
mtext(side=1, text=expression("SLA (mm"^2*" mg"^ -1*")"), line=1)
par(xpd=NA)
mtext(side=2, text=expression("Relative growth rate (ln(kg) year"^-1*")"), 
      line=1.25, las=0, at=relative.axis.point(-0.3, "y"))
par(xpd=FALSE)
box()
text(x=relative.axis.point(0.01, "x"), adj=0,
     y=relative.axis.point(0.94, "y"), labels="(e)", font=2)

# Plot raw data points
# with(plant.uncent[plant.cent$genus=="Acacia" &
#                plant.cent$aridity.index > quantile(plant.cent$aridity.index, 0.8) &
#                plant.cent$aridity.index < quantile(plant.cent$aridity.index, 1),],
#      points(mass.per.year ~ SLA, pch=16))
# 
# 
# with(plant.uncent[plant.cent$genus=="Acacia" &
#                     plant.cent$aridity.index > quantile(plant.cent$aridity.index, 0) &
#                     plant.cent$aridity.index < quantile(plant.cent$aridity.index, 0.2),],
#      points(mass.per.year ~ SLA, pch=1))

polygon(y=c(SLA.preds[[2]]$fit + 1.96*SLA.preds[[2]]$se.fit,
            rev(SLA.preds[[2]]$fit - 1.96*SLA.preds[[2]]$se.fit)),
        x=c(moist.SLA.seq, rev(moist.SLA.seq)),
        border=NA, col=rgb(0.996,0.941,0.332, 0.7))

points(y=SLA.preds[[2]]$fit, 
       x=moist.SLA.seq, col=rgb(0.616,0.569,0),
       type="l", lwd=1.5)

polygon(y=c(SLA.preds[[1]]$fit + 1.96*SLA.preds[[1]]$se.fit,
            rev(SLA.preds[[1]]$fit - 1.96*SLA.preds[[1]]$se.fit)),
        x=c(dry.SLA.seq, rev(dry.SLA.seq)),
        border=rgb(0.616,0.569,0), col=rgb(1,1,1,0.4))

points(y=SLA.preds[[1]]$fit, 
       x=dry.SLA.seq, col=rgb(0.616,0.569,0),
       type="l", lwd=1.5)

text(y=c(rev(SLA.preds[[1]]$fit)[1], rev(SLA.preds[[2]]$fit)[1]),
     x=c(dry.SLA.seq[200], moist.SLA.seq[200]),
     labels=c("Dry", "Moist"), pos=4, offset=0.2,
     col = rgb(0.616,0.569,0))
close.screen(5)

#                               Eucalyptus callout ####
MH.df<-as.data.frame(matrix(0, nrow=200, ncol=length(model.vars)))
colnames(MH.df)<-model.vars

MH.df.list<-list(MH.df, MH.df)

MH.df.list<-lapply(1:length(MH.df.list), function(x){
  temp<-MH.df.list[[x]]
  temp[,"aridity.index"]<-arid.quantile[x]
  
  if(x==1){
    temp[,"MH"]<-with(plant.cent[plant.cent$genus=="Eucalyptus" &
                                   plant.cent$aridity.index > quantile(plant.cent$aridity.index, 0) &
                                   plant.cent$aridity.index < quantile(plant.cent$aridity.index, 0.2),], 
                      seq(min(MH), max(MH), length.out=200))
  } else {
    temp[,"MH"]<-with(plant.cent[plant.cent$genus=="Eucalyptus" &
                                   plant.cent$aridity.index > quantile(plant.cent$aridity.index, 0.8) &
                                   plant.cent$aridity.index < quantile(plant.cent$aridity.index, 1),], 
  ...
  temp[,"genus"]<-factor(2)
  levels(temp[,"genus"])<-c("Acacia", "Eucalyptus")
  temp[1:dim(temp)[1],"genus"]<-"Eucalyptus"
  return(temp)
})

MH.preds<-lapply(MH.df.list, function(x){
  data.frame(predict(genera.m.arid, newdata=x, se.fit=TRUE, re.form=NA))
})

dry.MH.seq<-with(plant.uncent[plant.cent$genus=="Eucalyptus" &
                                plant.cent$aridity.index > quantile(plant.cent$aridity.index, 0) &
                                plant.cent$aridity.index < quantile(plant.cent$aridity.index, 0.2),], 
                 seq(min(MH), max(MH), length.out=200))

moist.MH.seq<-with(plant.uncent[plant.cent$genus=="Eucalyptus" &
                                  plant.cent$aridity.index > quantile(plant.cent$aridity.index, 0.8) &
                                  plant.cent$aridity.index < quantile(plant.cent$aridity.index, 1),], 
                   seq(min(MH), max(MH), length.out=200))
...
par(mar=c(0,0,0,0), ps=8, tck=-0.04, mgp=c(3,0.4,0), las=1)
plot(x=NULL, y=NULL, xlim=c(1.25,4.5),
     ylim=c(-0.05, 0.5), axes=FALSE, xlab="", ylab="")

axis(side=1, at=log(c(5, 10, 20, 50)), labels=c(5, 10, 20, 50),
     mgp=c(3,0,0))
axis(side=1, at=log(c(seq(1,9,1),seq(10,90,10))), labels=NA, tck=-0.02)
axis(side=2, at=c(0, 0.2, 0.4))
mtext(side=1, text="Maximum height (m)", line = 1)
box()
text(x=relative.axis.point(0.01, "x"), adj=0,
     y=relative.axis.point(0.94, "y"), labels="(f)", font=2)

polygon(y=c(MH.preds[[1]]$fit + 1.96*MH.preds[[1]]$se.fit,
            rev(MH.preds[[1]]$fit - 1.96*MH.preds[[1]]$se.fit)),
        x=c(dry.MH.seq, rev(dry.MH.seq)),
        border=rgb(0.157,0.31,0.184), col=rgb(1,1,1,0.6))

points(y=MH.preds[[1]]$fit, 
       x=dry.MH.seq, col=rgb(0.157,0.31,0.184),
       type="l", lwd=1.5)

polygon(y=c(MH.preds[[2]]$fit + 1.96*MH.preds[[2]]$se.fit,
            rev(MH.preds[[2]]$fit - 1.96*MH.preds[[2]]$se.fit)),
        x=c(moist.MH.seq, rev(moist.MH.seq)),
        border=NA, col=rgb(0.38,0.525,0.404, 0.4))

points(y=MH.preds[[2]]$fit, 
       x=moist.MH.seq, col=rgb(0.157,0.31,0.184),
       type="l", lwd=1.5)

text(y=c(rev(MH.preds[[1]]$fit)[100] + 1.96* MH.preds[[1]]$se.fit[100], 
         rev(MH.preds[[2]]$fit)[150] + 1.96 * MH.preds[[2]]$se.fit[150]),
     x=c(dry.MH.seq[100], moist.MH.seq[150]),
     labels=c("Dry", "Moist"), col=rgb(0.157,0.31,0.184),
     pos=3, offset=0.2)
